...
SELECT P.NOMBRE, P.APELLIDO, extract(year from E.FECHA), extract(month from E.FECHA), X.T
FROM PACIENTE P
    inner join EVALUACION E on E.ID_PACIENTE=P.ID_PACIENTE
JOIN 
(
    SELECT PT.ID_PACIENTE, count(PT.ID_TRATAMIENTO) T
    FROM PACIENTE_TRATAMIENTO PT  
    GROUP BY PT.PACIENTE_TRATAMIENTO
    HAVING count(PT.ID_TRATAMIENTO)>=1) X
on X.ID_PACIENTE = P.ID_PACIENTE
ORDER BY X.T DESC